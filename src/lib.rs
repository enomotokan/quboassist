...
fn quad_todict(qubo: &Quad, binindex_index: Vec<Vec<usize>>) -> PyResult<HashMap<((usize, usize), (usize, usize)), f32>> {
    ...
    // let indexes: Vec<((usize, usize), (usize, usize))> = qubo.index_list.clone().into_par_iter().map(|i| qubo.index_list_list[i].clone().into_iter().map(|j| ((binindex_index[i][0], binindex_index[i][1]), (binindex_index[j][0], binindex_index[j][1]))).collect::<Vec<_>>()).collect::<Vec<_>>().concat();
    // let values: Vec<f32> = qubo.index_list.clone().par_iter().flat_map(|&i| qubo.coef_list_list[i].clone()).collect::<Vec<_>>();
...
#[pymodule]
fn pyfunc_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
